...
class _SimpleCNN(torch.nn.Module):
    ...
    def forward(self, x):
        out = x  #.cuda()
        #if self.useTransform: out = self.transform(out)
        out = self.features(out)
        out = out.view(out.size(0), -1)
        out = self.drop0(out)
        if self.numClasses == 1: out = torch.sigmoid(self.forward0(out))
        if self.numClasses >  1: out = F.softmax(self.forward0(out))
...
def SimpleCNN(imgSize, dropOutRate, numClasses): 
    return _SimpleCNN(imgSize = imgSize, blocks = [32, 48, 32], dropOutRate = dropOutRate, numClasses = numClasses)
